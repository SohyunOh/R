...
# ggplot(data, aes(x= age,  y= income , fill = age)) +
#   geom_col()
...
data %>% 
  ...
  ggplot(aes(x= age,  y= mean_income , fill = age)) + geom_col()
...
#3.시각화
#reorder는 데이터 의 정렬순서, vs scale_x_discrete 축 순서
ggplot(d2, aes(x = ageg, y = mean_income, fill = ageg ))+
  geom_col() +
  labs(title = "연령데별 급여평군", x = "연령대" , y = "급여평균") +
  scale_x_discrete(limits =  c("young","middle","old")) #x푹 순서


#실습 ####
# 연령대 및 성별에 따른 급여 차이 막대 그래프 (그룹핑을 2개)
...
ggplot(d3, aes(x = ageg , y = mean_income, fill= gender)) +
  geom_co1( position = "dodge")+ # position = "dodge" 는 분리 fill은 채워줌
  scale_x_discrete(limits =  c("young","middle","old"))+ #x푹 순서
  labs(title = "연령데별 급여평군", x = "연령대" , y = "급여평균")  
